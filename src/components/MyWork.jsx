...
function MyWork({ emailUsuario, onVerDetalle }) {
  ...
  if (cargando) {
    return (
      <div className="p-8 text-gray-400 flex justify-center">
      ...
      <div className="space-y-8 w-full">
        {sprintsAgrupados.length === 0 ? (
          <div className="text-gray-500 bg-gray-900/50 p-6 rounded-lg border border-gray-700 text-center">
            No tienes tareas pendientes ni sprints activos asignados por el
            momento.
          </div>
        ) : (
              ...
              <div className="mb-4 pb-3 border-b border-gray-700/50 flex flex-col md:flex-row md:items-start justify-between gap-2">
                <div>
                  <h3 className="text-xl font-bold text-white flex items-center gap-2">
                    {grupo.sprint
                      ? `Sprint N° ${grupo.sprint.numero || index + 1}`
                ...
                {grupo.sprint && (
                  <span className="text-sm font-medium text-blue-400 bg-blue-900/20 px-3 py-1 rounded-full whitespace-nowrap">
                    {formatearFecha(grupo.sprint.fecha_inicio)} —{" "}
                    {formatearFecha(grupo.sprint.fecha_fin)}
                  </span>
                )}
              </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-xs font-bold text-purple-400 uppercase tracking-wider">
                          {tarea.sprint_id?.proyecto_id?.nombre ||
                            "Proyecto Activo"}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${colorEstado(tarea.estado)} uppercase`}
                        >
                          {column_labels[tarea.estado] || "To Do"}
                        </span>
                      </div>
                      ...
                      <div className="mt-1">
                        <span
                          className={`uppercase text-xs tracking-wide ${getColorPrioridad(tarea.prioridad)}`}
                        >
